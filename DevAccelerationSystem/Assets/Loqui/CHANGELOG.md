...
### Changed
- Documented the package boundary: fallback-first localization is intentionally separate from translation-management services, remote-content backends, and logging frameworks.
- Clarified that the package remains on the `0.x` line until a deliberate public-API stability decision is made.
- Updated the adding-language guide and tracked consumer PlayMode catalog fixture to the self-contained schema-2 catalog API.

## [0.3.1] - 2026-07-14

### Fixed
- Declared the built-in `com.unity.modules.jsonserialize` dependency required by remote-override parsing.
